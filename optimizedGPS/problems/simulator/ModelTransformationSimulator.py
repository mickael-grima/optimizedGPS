...
__all__ = ["ModelTransformationSimulator"]

log = logging.getLogger(__name__)


class ModelTransformationSimulator(Simulator):
    """ this simulator consider groups of drivers: a group is entirely characterized by a starting node, an ending node,
        ...
        For more details read report3.pdf
    """
    def __init__(self, graph, paths):
        ...
        super(ModelTransformationSimulator, self).__init__(graph)
    ...
    def initialize(self, initial_time=0):
        super(ModelTransformationSimulator, self).initialize()
    ...
    def get_current_state(self):
        state = super(ModelTransformationSimulator, self).get_current_state()
    ...
    def moveToNextEdge(self):
        ...
        """
        moved_drivers = {}  # for each driver we save a tuple (previous edge, next edge)
            ...
            for e, clocks in self.clocks[i].iteritems():
                ...
                while j < len(clocks) and clocks[j] == 0:
                    ...
                    if e < len(self.paths[i]) - 2:
                        ...
                        self.time += clock
                        moved_drivers.setdefault(nxt_edge, 0)
                        moved_drivers[nxt_edge] += 1
                    else:  # we kill the driver
                        self.nb_driver -= 1
                    if e >= 0:
                        prv_edge = (self.paths[i][e], self.paths[i][e + 1])
                        moved_drivers.setdefault(prv_edge, 0)
                        moved_drivers[prv_edge] -= 1
                    j += 1
                # we delete the 0
                self.clocks[i][e] = self.clocks[i][e][j:]
        return moved_drivers
    ...
    def next(self):
        ...
        if self.has_next():
            ...
            moved_drivers = self.moveToNextEdge()  # move every 0.0 clocks to the next edge
            self.update_traffic(moved_drivers)
        else:
