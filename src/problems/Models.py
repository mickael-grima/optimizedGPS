...
class ContinuousTimeModelWithoutPath(Model):
    def __init__(self, graph, timeout=sys.maxint, horizon=1000, binary=True):
        if not binary:
            log.error("Not implemented yet")
            raise NotImplementedError("Not implemented yet")
        super(ContinuousTimeModelWithoutPath, self).__init__(graph, timeout=timeout, NumericFocus=3)
        self.initialize_drivers()
        self.hor = horizon
        self.vtype = GRB.BINARY if binary else GRB.CONTINUOUS
    ...
    def buildConstraints(self):
        ...
        count = {'starting-edges': 0, 'ending-edges': 0, 'initial-conditions': 0, 'non-visited-edges-constraints': 0,
                 ...
                 'ending-time-constraints': 0, 'no-cycle-constraints': 0}
        for driver in self.drivers:
    ...
    def setObjective(self):
        self.model.setObjective(
            quicksum(self.E[(n, driver.end), driver]
                     for driver in self.drivers for n in self.graph.predecessors_iter(driver.end)),
            GRB.MINIMIZE
        )
        log.info("Objective SETTED")
